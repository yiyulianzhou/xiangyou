...
class GroupController extends ComController
{
    ...
    public function del()
    {
        ...
        if (is_array($ids)) {
            ...
            $ids = implode(',', $ids);
            $map['id'] = array('in', $ids);
            if (M('admin_auth_group')->where($map)->delete()) {
                addlog('删除用户组ID：' . $ids);
    ...
    public function update()
    {
        ...
        if ($data['title']) {
            ...
            $data['rules'] = $rules;
            if ($id) {
                $group = M('admin_auth_group')->where('id=' . $id)->data($data)->save();
                if ($group) {
                ...
                M('admin_auth_group')->data($data)->add();
    ...
    public function edit()
    {
        ...
        $group = M('admin_auth_group')->where('id=' . $id)->find();
        if (!$group) {
            $this->error('参数错误！');
        }
        //获取所有启用的规则
        $rule = M('admin_auth_rule')->field('id,pid,title')->where('status=1')->order('o asc')->select();
    ...
    public function status()
    {
        ...
        $status = $group['status'];
        if ($status == 1) {
           $res = M('admin_auth_group')->data(array('status' => 0))->where('id=' . $id)->save();
        }
        if ($status != 1 ) {
            $res = M('admin_auth_group')->data(array('status' => 1))->where('id=' . $id)->save();
        }
        if ($res) {
